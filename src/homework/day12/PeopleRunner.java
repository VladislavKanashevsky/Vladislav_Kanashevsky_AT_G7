...
public class PeopleRunner {
    public static void main(String[] args) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("age.txt"))) {
            writer.write(String.valueOf(people.filter(s -> s.getAge() < 60).sorted((Comparator.comparing(Person::getName))).map(s -> new Person(s.getAge() + 4, s.getName())).mapToInt(Person::getAge).average()));
